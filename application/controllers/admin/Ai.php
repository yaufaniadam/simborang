...
class AI extends MY_Controller
{
	...
	{
		parent::__construct();
		$this->load->model('admin/ai_model', 'ai_model');
	}

	public function details($id)
	{
		$data['dokumen'] = $this->ai_model->get_dokumen_by_id($id);
	...
	{
		$data['ambil_dokumen'] = $this->ai_model->ambil_dokumen($id_kategori);
		$data['view'] = 'admin/borang/ai/index';
		$this->load->view('admin/layout', $data);
	}
	...
	public function store($kategori)
	{
		if ($this->input->post('submit')) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$result = $this->ai_model->add_evaluasi_borang($data);
				if ($result) {
